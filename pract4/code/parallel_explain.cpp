...
int main(int argc, char **argv) {
  ...
  порядок процессов в исходном коммуникатора в процессе определения
  нового коммуникатора. То есть процессы будут тем же самым
  множеством, они будут организованы в виде решетки, но их ранги в
  новом коммуникаторе не будут совпалать с рангами исходного
  коммуникатора. Это было предусмотрено? MPI может знать конфигурации
  компа, на котором будет запущена программа. И бывает удобно эту
  конфигурацию учесть.
  Если в самой декартовой топологии расположение процессов
  соответствует расположоению физических устройст, на которых эти
  ...
  */
  ...
  /*MPI_Cart_coords
    ...
    Из любого процесса можем получить координаты любого другого процесса
  */
  ...
  /*MPI_Cart_sub
    ...
    координатам проводить объединение. Для создания коммуникатора строки
    хотим объединять по первой координате, которая потом пропадет в
    результате объединения. А вторая останется. И эта оставшаяся
    координата будет единственной в оставшихся одномерных расщепленных
    коммуникаторов. То есть объединение производится по координатам,
    которые мы убираем.
  */
  MPI_Comm commRow;
  ...
  /*
    Раздача матрицы А по горизонтальным полосам на вертикальную линейку
    процессов (0;0), (1;0), (2;0), …, (p1 - 1; 0) при помощи MPI_Scatter
  */
  ...
  /*
    Каждый из процессов в левой вертикальной колонке ( (1;0), (2;0), …,
    (p1 - 1; 0) ) при помощи MPI_Bcast раздает свою полосу матрицы A
    всем процессам своей горизонтали. Т.е. процесс (1;0) раздает свою
    полосу процессам (1;1), (1;2), ...
  */
  ...
  /*
    Определение нового производного типа данных для выбора из матрицы B
    вертикальных полос
  */
  ...
  */
  ...
  /*MPI_type_commit
    ...
    дальшейшем принимают новые типы (для определения других новых типов
    или посылки/приёма данных)
    *если новый тип хотим использовать при пересылке сообщений между
    процессами, то его необходимо дополнительно "зрарегистрировать",
    вызвав для него функцию MPI_type_commit
  */
  ...
  */
  ...
  /*
    Раздача матрицы B по вертикальным полосам на горизонтальную линейку процессов
    (0;0), (0;1), (0;2), …, (0; p2 – 1).
    Здесь происходит прием данных на нулевой столбец решётки
  */
  ...
  */
  ...
  /*
    Перемножаем столбец матрицы А на столбец матрицы В
  */
